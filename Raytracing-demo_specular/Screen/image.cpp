...
void image::set_pixel(int x, int y, const color& c)
{
    ...
    uint32_t col = SDL_MapRGBA(data -> format,
...
void image::draw_line(int x1, int y1, int x2, int y2, const color& c)
{
    ...
    if(dx > dy)
    {
        accumulated_error = dx / 2;
        for(i = 0; i < dx; i++)
        {
            x += xincr;
            accumulated_error += dy;
            if(accumulated_error > dx)
            {
                accumulated_error -= dx;
                y += yincr;
            }
            set_pixel(x, y, c);
        }
    ...
    {
        accumulated_error = dy / 2;
        for(i = 0; i < dy; i++)
        {
            y += yincr;
            accumulated_error += dx;

            if(accumulated_error > dy)
            {
                accumulated_error -= dy;
                x += xincr;
            }
...
{
    SDL_Rect r = {(Sint16) x1, (Sint16) y1,
                  (Uint16) (x2 - x1 + 1), (Uint16) (y2 - y1 + 1)
                 };
    SDL_FillRect(data, &r,
                 SDL_MapRGBA(data->format,
                             c.get_red(), c.get_green(),
                             c.get_blue(), c.get_alpha()));
}
